...
function createHeaderTemplate (destinationSet, totalPrice, dateStart, dateEnd) {
  if (destinationSet.length < 1 || !totalPrice || !dateStart || !dateEnd) {
    return '';
  }
  ...
  const titleOneDestination = `<h1 class="trip-info__title">${destinationSet.at(0)}</h1>`;
  const titleFewDestinations = `<h1 class="trip-info__title">${destinationSet.at(0)} &mdash; ${isDestinationCountLimitExceeded ? '...' : destinationSet.at(1)} &mdash; ${destinationSet.at(-1)}</h1>`;

  return `<section class="trip-main__trip-info  trip-info">
            <div class="trip-info__main">
              ${destinationSet.length === 1 ? titleOneDestination : titleFewDestinations}
